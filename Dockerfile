...
RUN cd "$MQTT_SN_REPO"-master/MQTTSNGateway && sed -i "s/^GatewayUDP6Hops=.*$/GatewayUDP6Hops=64/" gateway.conf
RUN cd "$MQTT_SN_REPO"-master/MQTTSNGateway && sed -i "s/^GatewayUDP6Port=.*$/GatewayUDP6Port=47193/" gateway.conf
RUN cd "$MQTT_SN_REPO"-master/MQTTSNGateway && sed -i "s/^GatewayUDP6Broadcast=.*$/GatewayUDP6Broadcast=ff33:40:fdde:ad00:beef:0:0:1/" gateway.conf
